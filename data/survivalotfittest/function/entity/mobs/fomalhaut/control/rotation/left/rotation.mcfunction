# 必要回転数を減算
scoreboard players add @s Choco.SotF.ScoreStorage.1 9
...
scoreboard players operation #Temp Choco.SotF.ScoreStorage.0 *= #-1 Choco.SotF.Math
execute if score #Temp Choco.SotF.ScoreStorage.0 matches 10.. run scoreboard players set #Temp Choco.SotF.ScoreStorage.0 9 

# 回転
execute on vehicle if entity @s[type=item_display,tag=aj.fomalhaut.root] rotated as @s run function survivalotfittest:entity/mobs/fomalhaut/control/rotation/left/set_rotation