...
#地上エンダーマンのみ常時敵対
execute at @s if dimension overworld if entity @e[tag=Choco.SotF.Enemies_Target,distance=..32,predicate=!survivalotfittest:if_invisible,predicate=!survivalotfittest:if_calm] run data modify entity @s AngryAt set from entity @e[tag=Choco.SotF.Enemies_Target,sort=nearest,limit=1] UUID
