...
execute at @s unless block ~ ~-0.4 ~ #survivalotfittest:can_through if score @s Choco.SotF.ScoreStorage.3 matches 40.. align y rotated ~ 0 run function survivalotfittest:entity/mobs/sculk_star/fight/shockwave


execute store result entity @s Rotation[1] float -1 run data get entity @s Rotation[1]
execute at @s unless block ^ ^ ^0.4 #survivalotfittest:can_through rotated as @s run rotate @s ~180 ~
