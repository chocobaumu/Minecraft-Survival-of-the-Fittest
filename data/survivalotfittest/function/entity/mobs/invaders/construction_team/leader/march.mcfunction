execute as @n[type=marker,tag=Choco.SotF.Invasion.FrontLine,tag=Choco.SotF.Conquered,tag=Choco.SotF.Temp] store result score #Invasion.FrontLine.Pos Choco.SotF.Pos.1 run data get entity @s Pos[1]
scoreboard players operation @s Choco.SotF.Pos.1 -= #Invasion.FrontLine.Pos Choco.SotF.Pos.1

execute as @s facing entity @n[type=marker,tag=Choco.SotF.Invasion.FrontLine,tag=Choco.SotF.Conquered,tag=Choco.SotF.Temp] feet run rotate @s ~ ~
...
# 設置(橋)
execute if score @s Choco.SotF.Pos.1 matches 0 rotated as @s rotated ~ 0 positioned ^ ^-1 ^1 if block ~ ~ ~ #survivalotfittest:can_through run function survivalotfittest:summon/misc/placed_block {"block":"stone_bricks"}

# ジャンプ(積み上げ用)
execute if score @s Choco.SotF.Pos.1 matches ..-1 if score @s Choco.SotF.tick.1 matches 10.. run data merge entity @s {Motion:[0.0, 0.45, 0.0]}