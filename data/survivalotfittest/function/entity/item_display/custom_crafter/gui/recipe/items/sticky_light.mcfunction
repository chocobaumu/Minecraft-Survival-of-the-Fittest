...
item replace block ~ ~ ~ container.11 with glow_berries[custom_data={SotF:GUIItem}]
item replace block ~ ~ ~ container.12 with air
item replace block ~ ~ ~ container.19 with copper_ingot[custom_data={SotF:GUIItem}]
item replace block ~ ~ ~ container.20 with air
item replace block ~ ~ ~ container.21 with air

# 完成品部分
item replace block ~ ~ ~ container.16 with warped_fungus_on_a_stick[custom_data={SotF:GUIItem},item_model="survivalotfittest:sticky_light",item_name="スティッキーライト",rarity="common",!max_damage,max_stack_size=24] 2
